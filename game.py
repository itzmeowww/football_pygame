...
def on_connect(self, client, userdata, rc):
    ...
    print("Subscribed to " + str(channel1))


def on_message(client, userdata, msg):
    globalvar = int(msg.payload.decode("utf-8", "strict"))
    print(globalvar)
    game.var = globalvar
...
class Ball:
    ...
    def update(self, screen):
        print(self.all_angle, self.angle)
        if self.all_angle == self.angle:
            self.angle = self.angle
        elif self.all_angle  < self.angle:
            self.x = self.x + self.speed
            self.angle = self.angle-5
            self.angle = self.angle%360
            self.Img = pygame.transform.rotate(self.old_Img,self.angle)
        elif self.all_angle > self.angle:
            self.x = self.x - self.speed
            self.angle = self.angle+5
            self.angle = self.angle%360
...
class Game:  
    ...
    def run(self):
        ...
        self.ball.all_angle = self.ball.all_angle + 5*self.var
        self.var = 0
